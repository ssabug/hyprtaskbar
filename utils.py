...
def getHomeDir():
    ...
    programConfigDirectory="hyprtaskbar";
    configSubDir="";
...
CONFIG_FILE=os.path.join(getHomeDir(),"config.json");
...
def log(msg,source='NONE',severity='INFO',sameline=False):  
    debug=getGlobalParam("debug");
    fileLogging=getGlobalParam("fileLogging");
    logFile=getGlobalParam("logFile");
